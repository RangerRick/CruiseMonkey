...
.controller('CMSeamailCtrl', ($interval, $ionicPopover, $ionicPopup, $ionicScrollDelegate, $log, $q, $rootScope, $scope, $stateParams, MuffleService, SettingsService, Twitarr, UserService) => {
	...
	$scope.isMuffled = (username) => {
		return MuffleService.isMuffled(username);
	};

	$scope.getClass = (message) => {
		const classes = [];
		if (MuffleService.isMuffled(message.author.username)) {
			classes.push('muffled');
			classes.push(message.expanded ? 'expanded' : 'not-expanded');
		} else {
			classes.push('not-muffled');
			classes.push('expanded');
		}
		return classes.join(' ');
	};

	$scope.messageClicked = (message) => {
	...
	$scope.doRefresh = () => {
		...
		$scope.user = UserService.get();
		return MuffleService.ready.then(() => {
			return Twitarr.getSeamailMessages($stateParams.id);
		}).then((res) => {
	...
	$scope.pullToRefresh = (keepPosition) => {
		return $scope.doRefresh(keepPosition).finally(() => {
			$rootScope.$broadcast('cruisemonkey.muffles.refresh');
		});
	};
...
.controller('CMSeamailsCtrl', ($interval, $ionicLoading, $ionicScrollDelegate, $log, $q, $rootScope, $scope, MuffleService, NewSeamail, SeamailService, SettingsService, UserService) => {
