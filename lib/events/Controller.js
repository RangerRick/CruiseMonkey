(function() {
	...
	.controller('CMEventsCtrl', function($filter, $ionicActionSheet, $ionicFilterBar, $ionicModal, $ionicPopover, $ionicPopup, $ionicScrollDelegate, $log, $q, $scope, $state, $timeout, EditEvent, EventService, kv, UserService) {
		...
		$scope.$on('cruisemonkey.notify.eventUpdated', function(ev, newEvent) {
			$scope.doRefresh();
		});
		$scope.$on('cruisemonkey.notify.eventAdded', function(ev, newEvent) {
			$scope.doRefresh();
		});
