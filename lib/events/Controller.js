...
.controller('CMEventsCtrl', ($filter, $ionicFilterBar, $ionicLoading, $ionicPopover, $ionicScrollDelegate, $localStorage, $log, $q, $scope, $timeout, EventService, kv, UserService) => {
	...
	const setEventType = (type) => {
		...
		kv.set('cruisemonkey.events.type', type);
	};
	...
	$scope.setEventType = (type) => {
		...
		if (getEventType() !== type) {
			showLoading();
			setEventType(type);
			$scope.doRefresh(false, false).then(() => {
				$timeout(() => {
					$scope.goToNow();
				});
			});
	...
	$scope.doRefresh = (cached = false, scroll = false) => {
		if (!$scope.isVisible) {
			return $q.when();
		}
		...
		$scope.$evalAsync(() => {
			UserService.waitForReady().then((user) => {
				if (user && user.loggedIn) {
					$scope.eventTypes = ['all', 'official', 'shadow', 'followed'];
				} else {
					$scope.eventTypes = ['all', 'official', 'shadow'];
				}
				console.log('refresh: currentevent type=', getEventType());
				console.log('event types:', $scope.eventTypes);
	...
	const collapsed = {};
	kv.get('cruisemonkey.events.collapsed').then((col) => {
		Object.assign(collapsed, col);
	});

	$scope.toggleCollapse = (index) => {
		const entry = $scope.events[index];
		if (entry) {
			const daystamp = entry.day.format('YYYY-MM-DD');
			collapsed[daystamp] = !collapsed[daystamp];
			kv.set('cruisemonkey.events.collapsed', collapsed);
			$ionicScrollDelegate.$getByHandle('event-scroll').scrollBy(0, 0, true);
		}
	};
	...
	$scope.isCollapsed = (index) => {
		...
		if (entry) {
			if (entry.day) {
				return Boolean(collapsed[entry.day.format('YYYY-MM-DD')]);
			} else {
				return Boolean(collapsed[entry.getStart().format('YYYY-MM-DD')]);
			}
		}
		return false;
	};

	$scope.getIconClass = (index) => {
		return 'icon ion-chevron-' + ($scope.isCollapsed(index)? 'right' : 'down');
	};
	...
	$scope.$on('$ionicView.beforeEnter', () => {
		...
		kv.get('cruisemonkey.events.type').then((type) => {
			$scope.$storage['cruisemonkey.events.type'] = type;
			console.log('loaded type:', type);
		}).catch(() => {
			$scope.$storage['cruisemonkey.events.type'] = defaultEventType;
			console.log('loaded default type:', defaultEventType);
		}).finally(() => {
