...
.factory('EventService', ($log, $q, $rootScope, kv, Twitarr, UserService) => {
	...
	const doCachedQuery = (where) => {
		return $q.when(events.filter(where).sort(sortEvent));
	};
	...
	const getUserEvents = (cached) => {
		...
		const query = function UserEvents(ev) {
			return ev.getAuthor().username === user.username;
		};
		return cached? doCachedQuery(query) : doQuery(query);
	};
	...
	const getEventForTime = (time, eventList) => {
		...
		if (eventList && eventList.length > 0) {
			// events that last longer than 5 hours are probably "all day"
			const lengthCutoff = 60 * 60 * 5;
			...
			for (let i = 0, len=eventList.length; i < len; i++) {
				...
				if (entry instanceof CMDay) {
					...
					if (end - start >= lengthCutoff) {
						console.log('skipping event because it is too long:', entry);
						continue;
					}
