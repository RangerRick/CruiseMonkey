(function() {
	...
	.controller('CMAboutCtrl', ['$scope', '$window', 'config.app.version', 'config.app.build', function($scope, $window, version, build) {
		...
		$scope.openPaypal = function() {
			$window.open('https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=4GNPSE8RNFGME', '_system', 'closebuttoncaption=Close,transitionstyle=fliphorizontal');
		};
	}]);
}());
