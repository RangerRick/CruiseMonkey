(function() {
	...
	.factory('LocalNotifications', function($cordovaLocalNotification, $log, $q, $rootScope, $window, Cordova, kv) {
		...
		var hasLocalNotifications = function() {
			return Cordova.inCordova().then(function() {
				return $window && $window.cordova && $window.cordova.plugins && $window.cordova.plugins.notification && $window.cordova.plugins.notification.local;
			}, function() {
				return false;
			});
		};
	...
	.factory('Notifications', function($cordovaDialogs, $cordovaSpinnerDialog, $ionicLoading, $ionicPopup, $log, $q, $rootScope, $timeout, $window, ionicToast, LocalNotifications) {
		...
		$rootScope.$on('cruisemonkey.notify.toast', function(ev, toast) {
			...
			ionicToast.show(toast.message, 'top', false, timeout);
		});
