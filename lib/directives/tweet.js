...
.directive('cmTweet', ($log, $rootScope, ImageViewer, TweetEditor, UserService) => {
	...
	return {
		...
		link: (scope, el, attrs) => {
			scope.allowRecursion = attrs.allowRecursion !== undefined && attrs.allowRecursion.trim() === 'true';
			scope.hideControls = attrs.hideControls !== undefined && attrs.hideControls.trim() === 'true';

			if (attrs.allowRecursion !== undefined && attrs.allowRecursion.trim() === 'true') {
				scope.allowRecursion = true;
			}
			if (attrs.hideControls !== undefined && attrs.hideControls.trim() === 'true') {
