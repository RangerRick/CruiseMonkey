...
const stringMap = (obj) => {
	...
	for (let i=0, len=keys.length, key; i < len; i++) {
		key = keys[i];
		if (Object.prototype.hasOwnProperty.call(obj, key) && key !== 'data') {
			obj[key] = '' + obj[key];
		}
	}
	return obj;
...
ionic.Platform.ready(() => {
	module.factory('cmHTTP', ($http, $injector, $log, $q, $window, SettingsService, UserService) => {
		const requestTimeout = parseInt($injector.get('config.request.timeout'), 10) * 1000;
