...
var popoverUrl = require('./chooser.html');
...
.factory('Photos', function($compile, $cordovaCamera, $document, $ionicPopover, $ionicPopup, $log, $q, $rootScope, $window, FileItem, FileUploader, SettingsService, Twitarr, UserService) {
	...
	var findKey = function(obj, keyName) {
		if (obj) {
	...
	var getPhotoId = function(response) {
		if (response) {
			var status = findKey(response, 'status');
			var id = findKey(response, 'id');
			var hash = findKey(response, 'md5_hash');
			var photo = findKey(response, 'photo');
			if (status === 'ok') {
				if (id) return id;
				if (hash) return hash;
