...
const popoverUrl = require('./chooser.html');
...
.factory('Photos', function($compile, $cordovaCamera, $document, $ionicPopover, $ionicPopup, $log, $q, $rootScope, $window, FileItem, FileUploader, SettingsService, Twitarr, UserService) {
	...
	const findKey = function(obj, keyName) {
		if (obj) {
	...
	const getPhotoId = function(response) {
		if (response) {
	...
	const resolveUrl = function(unresolved) {
		...
		const postPhoto = function(url) {
			$scope.$evalAsync(function() {
				...
				tw(url).then(function(res) {
					const response = angular.fromJson(res.response);
	...
	$scope.uploadFile = function(image) {
		...
		if (image && image.data) {
			...
			return SettingsService.getTwitarrRoot().then(function(twitarrRoot) {
				const options = new FileItem($scope.uploader, file);
				if ($scope.isProfile) {
