...
function CMEvent(data) {
	...
	self._rawdata = {};
	if (data) {
		Object.assign(self._rawdata, data);
	}
...
CMEvent.prototype.getDisplayTime = function() {
	...
	if (start) {
		...
		var tzString = end? end.format('z') : start.format('z');
		if (tzString) {
			ret += ' ' + tzString;
		}
		return ret;
...
CMEvent.prototype.toString = function() {
	return 'CMEvent[id=' + this._rawdata.id + ',summary=' + this._rawdata.title + ',favorites=' + this.getFavorites() + ',shared=' + (this.isShared()? 'true':'false') + ',official=' + (this.isOfficial()? 'true':'false') + ']';
};
