...
.factory('ImageCache', ($log, $q, $rootScope, Cordova) => {
	...
	const getMetadata = (file) => {
		...
		file.getMetadata((metadata) => {
			$rootScope.$evalAsync(() => {
				$log.debug('getMetadata: got: ' + angular.toJson(metadata));
				deferred.resolve(metadata);
			});
		}, (err) => {
			$rootScope.$evalAsync(() => {
				$log.error('getMetadata: error: ' + angular.toJson(err));
				deferred.reject(err);
			});
		});
	...
	const downloadUrl = (url) => {
		...
		return fs.download(url, localPath, {
			...
			$log.debug('ImageCache.downloadUrl: saved to cache: ' + localPath);
			return entry;
		}, (err) => {
	...
	const getImageCordova = (url) => {
		...
		return fs.exists(path).then((exists) => {
			...
			return $q.when(fs.toURL(entry)).then((url) => {
				const converted = url; // window.Ionic.WebView.convertFileSrc(url.replace(/^file:\/\//).replace(/^undefined/, 'http://localhost/_file_'));
				$log.debug(`getImageCordova: final resolve: ${converted}`);
				return converted;
			});
		}).catch((err) => {
