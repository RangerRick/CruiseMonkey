...
.run(($cordovaCamera, Beacon, Conduct, EmojiService, EventService, Notifications, $cordovaKeyboard, $ionicHistory, $log, $rootScope, $sce, $state, $window, Cordova, kv, LocalNotifications, SettingsService, Twitarr, UpgradeService, UserService) => {
			...
			if (stateSections.length > 1) {
				...
				if (currentView.stateName === baseView) {
					...
					if (view.stateName === 'tab.info-settings') {
						var deregister = $rootScope.$on('$ionicView.enter', (ev, entered) => {
