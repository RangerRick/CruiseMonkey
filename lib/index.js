(function () {
	...
	.config(function($stateProvider, $urlRouterProvider, $compileProvider, $ionicConfigProvider, $ionicFilterBarConfigProvider, $logProvider, ImgCacheProvider) {
		if (isMobile) {
		...
		if (cordova && cordova.file && cordova.file.cacheDirectory) {
			var localCache = cordova.file.cacheDirectory + 'images';
			if (cordova.file.externalCacheDirectory) {
				localCache = cordova.file.externalCacheDirectory + 'images';
			}
			console.log('local cache folder: ' + localCache);

			ImgCacheProvider.setOption('localCacheFolder', cordova.file.cacheDirectory + 'imgcache');
		}
		*/
