(function () {
	...
	.run(function($http, $cordovaCamera, $cordovaKeyboard, $cordovaSplashscreen, $injector, $ionicHistory, $ionicModal, $ionicPlatform, $ionicPopover, $ionicPopup, $log, $q, $rootScope, $sce, $state, $templateCache, $timeout, $window, Cordova, kv, EmojiService, EventService, LocalNotifications, Notifications, SettingsService, Twitarr, UpgradeService, UserService) {
		...
		kv.get('cruisemonkey.navigation.current-view').then(function(cv) {
			...
			if (view && view.stateName) {
				if (view.stateName === currentView.stateName && angular.toJson(view.stateParams) === angular.toJson(currentView.stateParams)) {
					$log.debug('We are already on the current view.');
					return;
				}
