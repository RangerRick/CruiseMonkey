...
const templates = {
	...
	gameList: require('./games/list.html'),
...
[
	...
	'cruisemonkey.controllers.Games',
...
.config(($stateProvider, $urlRouterProvider, $compileProvider, $ionicConfigProvider, $ionicFilterBarConfigProvider, $logProvider) => {
	...
	$stateProvider
		...
		.state('tab.info-games', {
			url: '/info/games',
			views: {
				'tab-info': {
					templateUrl: templates['gameList'],
					controller: 'CMGameSearchCtrl'
				}
			}
		})
...
.run(($cordovaCamera, BackgroundManager, Conduct, EmojiService, EventService, Notifications, $ionicHistory, $log, $q, $rootScope, $sce, $state, $timeout, $window, Cordova, kv, LocalNotifications, SettingsService, Twitarr, UpgradeService, UserService) => {
	...
	$rootScope.isSectionEnabled = (section) => {
		...
		if (cmSectionValue !== undefined) {
			return Boolean.of(cmSectionValue);
		}
		return Boolean.of($rootScope.sections[section]);
	};
	...
	const navigateTo = (view) => {
		$rootScope.$evalAsync(() => {
			...
			$log.debug('Main.navigateTo: stateSections=' + angular.toJson(stateSections));
	...
	inCordova.then((inCordova) => {
		if (inCordova) {
			...
			*/
			...
			$log.info('Configuring badge plugin.');
