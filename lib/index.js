(function () {
			...
			.state('tab.seamail-view', {
				...
				views: {
					'tab-seamail': {
						...
						resolve: {
							seamail: function($stateParams, SeamailService) {
	...
	.run(function($http, $cordovaCamera, $cordovaKeyboard, $cordovaSplashscreen, $injector, $ionicModal, $ionicPlatform, $ionicPopover, $ionicPopup, $log, $q, $rootScope, $sce, $state, $templateCache, $timeout, $window, ImgCache, util, Cordova, kv, EmojiService, EventService, LocalNotifications, Notifications, SettingsService, Twitarr, UpgradeService, UserService) {
		...
		kv.get('cruisemonkey.navigation.current-view').then(function(cv) {
			...
			if (cv && cv !== '') {
				$timeout(function() {
					$log.debug('restoring view: ' + currentView);
					util.go(currentView);
				});
			}
		});
