(function() {
	...
	.controller('CMProfileCtrl', function($ionicPopup, $log, $q, $scope, Photos, SettingsService, Twitarr, UserService) {
		...
		$scope.uploadPic = function(pic) {
			$log.info('Upload Picture: ' + angular.toJson(pic));
			if (pic instanceof Array) {
				pic = pic[0];
			}
			Twitarr.postUserPhoto(pic).then(function(res) {
				$log.debug('res=' + angular.toJson(res));
				$scope.photo = res.data.files[0].photo;
		...
		$scope.fileSelected = function(file, files, errFiles, evt) {
			$log.debug('file=' + angular.toJson(file));
			$scope.$evalAsync(function() {
				$scope.uploadPic(file[0]);
			});
		};
