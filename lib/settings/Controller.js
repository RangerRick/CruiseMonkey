...
.controller('CMSettingsCtrl', ($log, $q, $rootScope, $scope, $timeout, SettingsService, UserService) => {
	...
	$scope.isUnchanged = () => {
		if ($scope.settings) {
			...
			const updatedAdvancedSync = Boolean.of($scope.settings.enableAdvancedSync);
			const existingAdvancedSync = Boolean.of(existing.enableAdvancedSync);

			const updatedShowPronouns = Boolean.of($scope.settings.showPronouns);
			const existingShowPronouns = Boolean.of(existing.showPronouns);

			return existingInterval === updatedInterval && existingAdvancedSync === updatedAdvancedSync && existingShowPronouns === updatedShowPronouns;
		}
		return false;
	};
	...
	$scope.resetSettings = (ev) => {
		...
		const newDefaults = SettingsService.getDefaultSettings();
		newDefaults.twitarrRoot = $scope.settings.twitarrRoot;
		newDefaults.twitarrRootSelector = $scope.settings.twitarrRootSelector;
		$log.debug('CMSettingsCtrl.resetSettings(): resetting to: ' + angular.toJson(newDefaults));
		$scope.settings = angular.copy(newDefaults);
	};
	...
	$scope.saveSettings = () => {
		...
		return $q.all([
			...
			SettingsService.setEnableAdvancedSync(Boolean.of($scope.settings.enableAdvancedSync)),
			SettingsService.setShowPronouns(Boolean.of($scope.settings.showPronouns)),
		]).then(() => {
			return init();
		});
	};
