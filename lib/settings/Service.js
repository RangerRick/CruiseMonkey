(function() {
	...
	.value('config.background.interval', 300)
	.value('config.twitarr.root', 'http://joco.rccl.com/')
	.value('config.twitarr.enable-cachebusting', false)
	...
	.factory('SettingsService', function($injector, $ionicPopup, $location, $log, $q, $rootScope, $window, kv, UpgradeService) {
		...
		var startCruise = datetime.create('2016-02-21 12:00');
		var endCruise   = datetime.create('2016-02-28 00:00');
		var now = datetime.create();
		if (now.isSameOrAfter(startCruise)) {
			defaultValue['twitarr.root'] = 'http://joco.rccl.com/';
		}
		...
		var init = function() {
			...
			kv.get('cruisemonkey.settings.onaboat').then(function(onaboat) {
				...
				if (now.isSameOrAfter(startCruise) && !onaboat) {
					$log.info('We\'re on a boat!  Setting to production twit-arr URL.');
					if ($rootScope.twitarrRoot !== defaultValue['twitarr.root']) {
						$ionicPopup.alert({
							title: 'On a Boat!',
							template: 'Setting your server to the shipboard address.',
							okText: 'Yay!'
						});
						$rootScope.$broadcast('cruisemonkey.wipe-cache');
						setTwitarrRoot(defaultValue['twitarr.root']);
					}
