...
.value('config.showPronouns', false)
...
.factory('SettingsService', ($injector, $log, $q, $rootScope, kv) => {
	...
	const defaultSettings = {
		...
		showPronouns: Boolean.of($injector.get('config.showPronouns')),
	};
	...
	const getEnableAdvancedSync = () => {
		return getProperty('enableAdvancedSync').then((enableAdvancedSync) => {
			if (enableAdvancedSync === undefined) {
				return Boolean.of($injector.get('config.enableAdvancedSync'));
			}
			return Boolean.of(enableAdvancedSync);
		});
	};

	const getShowPronouns = () => {
		return getProperty('showPronouns').then((showPronouns) => {
			if (showPronouns === undefined) {
				return Boolean.of($injector.get('configs.showPronouns'));
			}
			return Boolean.of(showPronouns);
		});
	};
	...
	const setShowPronouns = (show) => {
		return setProperty('showPronouns', Boolean.of(show));
	};
	...
	return {
		...
		getEnableAdvancedSync,
		setEnableAdvancedSync,
		getShowPronouns,
		setShowPronouns,
	};
});
