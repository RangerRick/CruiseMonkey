(function() {
	...
	.controller('CMTwitarrStreamCtrl', function($interval, $ionicActionSheet, $ionicLoading, $ionicPopup, $ionicScrollDelegate, $log, $q, $rootScope, $scope, $timeout, EmojiService, SettingsService, Twitarr, UserDetail, UserService, uuid4) {
			...
			return Twitarr.getStream().then(function(res) {
				...
				if (res && res.stream_posts && res.stream_posts.length > 0) {
					...
					if (keepPosition) {
						...
						if ($scope.isVisible) {
							$rootScope.$broadcast('cruisemonkey.notify.tabs.showMentions', false);
						}
					}
				}
