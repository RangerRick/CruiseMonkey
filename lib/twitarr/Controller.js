(function() {
	...
	var imageTemplate = require('ngtemplate!html!./image.html');
	...
	.controller('CMTweetCtrl', function($ionicModal, $log, $scope, UserDetail, tweet, twitarrRoot) {
		require.ensure(['moment'], function(require) {
			var moment = require('moment');
			tweet.timestamp = moment(tweet.timestamp);

			$log.debug('tweet = ' + angular.toJson(tweet));
			...
			$scope.showImage = function() {
				$ionicModal.fromTemplateUrl(imageTemplate, {
					scope: $scope
				}).then(function(modal) {
					$scope.modal = modal;
	...
	.controller('CMTwitarrStreamCtrl', function($q, $scope, $log, $interval, $timeout, $ionicLoading, $ionicScrollDelegate, EmojiService, SettingsService, Twitarr, UserDetail, UserService, uuid4) {
		...
		require.ensure(['moment'], function(require) {
			...
			processTweet = function(tweet) {
				...
				//console.log('processTweet: ' + angular.toJson(tweet));
				return tweet;
			};
		});
