...
.controller('CMTweetCtrl', ($ionicLoading, $ionicScrollDelegate, $log, $q, $rootScope, $scope, $stateParams, $timeout, ImageViewer, MuffleService, SettingsService, Twitarr, UserDetail, UserService) => {
	$scope.openUser = UserDetail.open;
	$scope.user = UserService.get();
	$scope.tweets = [];

	$scope.focusedTweet = $stateParams.id;
	...
	const drawTweets = (tweets) => {
		const newTweets = tweets.map((tweet) => {
			if (tweet.status === 'ok') {
				return decorateTweet(tweet.post);
			} else {
				return decorateTweet(tweet);
			}
		});
	...
	$scope.pullToRefresh = () => {
		return $scope.doRefresh().finally(() => {
	...
	$scope.$on('$ionicView.beforeEnter', () => {
		...
		return MuffleService.ready.then((muffles) => {
			$log.debug('muffles:', muffles);
			$timeout(() => {
