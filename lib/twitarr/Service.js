...
(function() {
	...
	.factory('Twitarr', function($interval, $ionicPlatform, $log, $q, $rootScope, $timeout, $window, BackgroundManager, cmHTTP, Cordova, kv, LocalNotifications, SettingsService, Upload, UserService) {
					...
					const fetchCallback = () => {
						$scope.$evalAsync(() => {
							...
							checkStatus().then(() => {
								...
								$log.error('Twitarr: Background fetch failed: ' + angular.toJson(err));
							}).finally(() => {
