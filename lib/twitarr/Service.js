(function() {
	...
	.factory('Twitarr', function($interval, $ionicPlatform, $log, $q, $rootScope, $timeout, $window, cmHTTP, Cordova, kv, LocalNotifications, SettingsService, Upload, UserService) {
		...
		var getAutocompleteUsers = function(searchPrefix) {
			...
			var url = 'api/v2/user/ac/' + searchPrefix;
		...
		var getStatus = function(shouldReset) {
			var user = UserService.get();
			if (!user.key || !user.loggedIn) {
		...
		var postSeamail = function(seamail) {
			...
			var url = 'api/v2/seamail?key=' + user.key;
		...
		var postSeamailMessage = function(id, text) {
			...
			var url = 'api/v2/seamail/' + id + '?key=' + user.key;
		...
		var like = function(tweetId) {
			...
			var url = 'api/v2/tweet/' + tweetId + '/react/like?key=' + user.key;
		...
		var unlike = function(tweetId) {
			...
			var url = 'api/v2/tweet/' + tweetId + '/react/like';
		...
		var getStreamByReacted = function(nextPage) {
			return getStream(nextPage, {
				reacted: true
			});
		};
		...
		var getTweet = function(id) {
			var url = 'api/v2/thread/' + id + '?app=plain';
		...
		var updateTweet = function(tweet) {
			...
			var url = 'api/v2/tweet/' + tweet.id + '?app=plain';
		...
		var getSections = function() {
			...
			return get(url).then(function(response) {
				...
				var ret = {};
				for (var i=0, len=response.data.sections.length, section; i < len; i++) {
		...
		var toggleStarred = function(username) {
			var url = 'api/v2/user/profile/' + username + '/star';
		...
		var getUserInfo = function(username) {
			var url = 'api/v2/user/profile/' + username;
		...
		var backgroundFetchCallback = function() {
			$log.info('Twitarr: Background fetch initiated.');
			checkStatus().then(function() {
				$log.info('Twitarr: Background fetch complete.');
				$timeout(function() {
					if (window && window.BackgroundFetch) {
						window.BackgroundFetch.finish();
					} else {
						$log.warn('$window.BackgroundFetch is unavailable!');
					}
				}, 100);
			}).catch(function(err) {
				$log.warn('Background fetch status check failed: ' + angular.toJson(err));
			});
		};

		var configureBackgroundFetch = function() {
			$ionicPlatform.ready(function() {
				if (window && window.BackgroundFetch && !$ionicPlatform.isAndroid()) {
					$log.info('Twitarr: Configuring background fetch.');
					var fetcher = window.BackgroundFetch;
					fetcher.configure(backgroundFetchCallback, function(err) {
						$log.debug('Twitarr: Background fetch call failed: ' + angular.toJson(err));
					});
				} else {
					$log.warn('Twitarr: Background fetch not found.');
				}
			});
		};
