...
.factory('Twitarr', ($interval, $ionicPlatform, $log, $q, $rootScope, $timeout, $window, BackgroundManager, cmHTTP, Cordova, kv, LocalNotifications, SettingsService, Upload, UserService) => {
	...
	const getSections = () => {
		...
		return get(url).then((response) => {
			...
			for (let i=0, len=response.data.sections.length, section; i < len; i++) {
				...
				ret[section.name] = Boolean(section.enabled);
			}
			return ret;
		}, (errorResponse) => {
			$log.error('Failed updateTweet(): ' + errorResponse.status + ': ' + angular.toJson(errorResponse.data));
	...
	const sendMentionNotification = (mention, count) => {
	...
	const sendAnnouncementNotification = (announcement, count) => {
	...
	const sendSeamailNotification = (seamail, count) => {
	...
	const sendSeamailNotifications = () => {
		const user = UserService.get();
		if (!user.key || !user.loggedIn) {
	...
	const checkStatus = () => {
		...
		if (user.loggedIn && user.key) {
			$log.debug('Twitarr.checkStatus: doing status check');
			return getAlerts(false).then((alerts) => {
				const new_mentions = [],
					new_announcements = [],
					new_seamails = [];
				let i, mention, announcement, seamail;
				...
				const total_updates = new_announcements.length + new_mentions.length + new_seamails.length;
				if ($window.cordova && $window.cordova.plugins && $window.cordova.plugins.notification && $window.cordova.plugins.notification.badge) {
					...
					$window.cordova.plugins.notification.badge.set(total_updates);
				}
				...
				$log.debug('Twitarr.checkStatus: complete.');
			});
		} else {
			$log.debug('Twitarr.checkStatus: skipping status check, user is not logged in.');
