			...
			if (!existingUser || force) {
				if (!inFlight[username]) {
					inFlight[username] = Twitarr.getUserInfo(username).then(function(updatedUser) {
						if (updatedUser.display_name && updatedUser.display_name !== updatedUser.username) {
							updatedUser.displayName = updatedUser.display_name;
							updatedUser.displayHandle = '(@' + username + ')';
						} else {
							updatedUser.displayName = '@' + username;
							updatedUser.displayHandle = '';
						}
