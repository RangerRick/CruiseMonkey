...
xdescribe('cruisemonkey.Database', function() {
	...
	describe('service#get(test-local)', function() {
		it('should pass if we could get a local database', function(done) {
	...
	describe('db#destroy(test-local)', function() {
		it('should pass if we got no failures destroying a database', function(done) {
			var db = _database.get('test-local');
			expect(db).toBeDefined();
	...
	describe('db#updateFrom()', function() {
		it('should pass if we can batch-sync one database to another', function(done) {
		...
		it('should copy only the things that match the given view', function(done) {
		...
		it('should use the username as key when querying', function(done) {
			createTestDb().then(function() {
				var remote    = _database.get(webroot + 'test-remote');
				var favorites = _database.get('test-favorites', {
					'view': {
	...
	describe('db#replicateFrom()', function() {
		it('should pass if we can batch-replicate one database to another', function(done) {
			var local = _database.get('test-local');
			var other = _database.get('test-other');
			local.destroy().then(function() {
				other.destroy().then(function() {
	...
	describe('db#query()', function() {
		it('should pass if we can get all test events with a query', function(done) {
			createTestDb().then(function() {
				var remote = _database.get(webroot + 'test-remote');
	...
	describe('db#get()', function() {
		it('should get the specific document we ask for', function(done) {
			createTestDb().then(function() {
				var remote = _database.get(webroot + 'test-remote');
	...
	xdescribe('Syncing', function() {
		...
		it('should pass if we can sync 30k events', function(done) {
			createTestDb().then(function() {
				var local  = _database.get('test-local');
				var remote = _database.get(webroot + 'test-remote');
